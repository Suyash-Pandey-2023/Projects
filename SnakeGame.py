...
def reset():
    global snake, snake_direction, food_pos, pen, high_score
...
# Function to show the high score pop-up and ask if the user wants to continue
def show_popup(score):
    response = messagebox.askquestion("Game over !!", f"Your score is: {score}\nDo you want to continue?")
    
    if response == 'yes':
        messagebox.showinfo("Response", f"Great! Let's keep going! {reset()}")
    else:
        messagebox.showinfo("Response", f"Goodbye! See you next time!")
        turtle.bye()
